defmodule Password do
  ...
  @symbols ~c"!@#$%^&*()-_=+[]{}|;:,.<>?"
  @words [
    "apple", "banana", "cherry", "date", "elephant", "frog", "giraffe", "house", "ice", "jungle", "kite", "lion", "monkey", "night", "orange", "penguin", "queen", "rabbit", "snake", "tiger", "umbrella", "village", "wolf", "xylophone",
    "yacht", "zebra", "ant", "ball", "cat", "dog", "egg", "fish", "goat", "hat", "igloo", "jar", "key", "leaf", "moon", "nest", "owl", "pig", "quilt", "rat", "sun", "tree", "urn", "van", "web", "box", "yak", "zip", "air", "bus", "cow", "drum", "ear", "fan", "gem", "hill", "ink", "jug", "kid", "log", "mat", "net",
    "oak", "pan", "pot", "rat", "sip", "tap", "urn", "vet", "wig", "yak", "zoo", "arch", "belt", "clap", "dusk", "ease", "fizz", "glow", "hint", "idea", "jolt", "knob", "lamb", "moth", "numb", "opus", "peek", "quip", "ramp", "skip", "twig"
  ]
...
defmodule CLI do
  def main(args) do
    ...
    opts =
      opts
      |> Enum.map(fn
        {:type, val} -> {:type, String.to_atom(val)}
        other -> other
      end)
      |> Enum.into(%{})

    password = Password.generate(opts)

    case opts[:file] do
      nil -> IO.puts(password)
      path -> File.write!(path, password <> "\n") && IO.puts("Password saved to #{path}")
    end
  end
end
